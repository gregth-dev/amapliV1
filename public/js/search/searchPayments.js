    ...
    .then(result => {
        ...
        Object.keys(paymentsList).forEach(key => {
            resultat[paymentsList[key].id] = {
                ...
                contract: paymentsList[key].contract + " ",
...
function tbodyConstruct(tbody, tab) {
    Object.keys(tab).forEach(key => {
        ...
        cell3.textContent = tab[key].contract;
        let cell4 = row.insertCell();
        cell4.textContent = tab[key].checkNumber;
        let cell5 = row.insertCell();
        cell5.textContent = tab[key].amount;
        let cell6 = row.insertCell();
        cell6.textContent = tab[key].depositDate;
        ...
        cell7.append(statusSpan);
        let cell8 = row.insertCell();
        cell8.classList.add('d-flex', 'justify-content-center');
        let checkPayment = Tools.createLink('warning', `/referent/paiement/remettre/${id}`, null, "Remettre le paiement", "fas fa-cash-register");
        checkPayment.classList.add('checkPayment');
        if (status.trim() === 'remis')
            checkPayment.classList.add('disabled');
        checkPayment.id = `${id}`;
        cell8.append(checkPayment);
    });
...
function filterData(searchTab, search) {
    return searchTab.filter(
        (a) => a.id.toLowerCase().includes(search.toLowerCase()) ||
            ...
            a.contract.normalize('NFD').replace(/[\u0300-\u036f]/g, "").toLowerCase().includes(search.normalize('NFD').replace(/[\u0300-\u036f]/g, "").toLowerCase()) ||
